...
const DEMO_STEPS = [
  { label: 'Loading 48,842 rows…',                  ms: 1200 },
  { label: 'Detecting column types…',                ms: 900  },
  { label: 'Building feature correlation graph…',    ms: 1500 },
  { label: 'Tracing relay chains (depth 4)…',        ms: 1800 },
  { label: 'Scoring paths with Vertex AI (AutoML)…', ms: 1200 },
  { label: 'Computing fairness metrics…',             ms: 900  },
  { label: 'Generating audit report…',                ms: 500  },
]
const DEMO_TOTAL_MS = DEMO_STEPS.reduce((s, x) => s + x.ms, 0) // 8000ms
...
function DemoProgress({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setPct(Math.min(99, ((Date.now() - startRef.current) / DEMO_TOTAL_MS) * 100))
    }, 80)
  ...
  return (
    <div className="mb-6 p-6 bg-slate-800/80 border border-slate-600 rounded-xl space-y-4">
      ...
      <div className="h-1.5 bg-slate-700 rounded-full overflow-hidden">
        <div className="h-full bg-gradient-to-r from-red-600 to-orange-400 rounded-full"
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {!columns.length && !demoLoading && (
        <div className="mb-6 p-4 bg-slate-800/60 border border-slate-600 rounded-xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={handleDemo}
              className="shrink-0 mt-1 px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-lg transition-colors whitespace-nowrap"
            >
              Run Demo →
            </button>
          </div>
